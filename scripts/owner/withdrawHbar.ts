...
async function main() {
    ...
    let gasLimit = 33_000
    let balance = +(await getHbarBalanceForId(HTS1400ContractId.toString()))
    let amountToWithdraw = Math.round(balance/2); // adjust as needed
    let tx = await hts1400.connect(ownerSigner).withdrawHbar(amountToWithdraw, {gasLimit: gasLimit})
